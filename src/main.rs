...
impl LogEntryStruct {
  ...
  pub fn parse_entry(self, dat: &Root, st_final: &mut HashSet<String>) {
    ...
    for sg_id in sgs_vec {
      ...
      let (xyz,source) = aws::ip_allowed_in_sg(&self.source_ip, sg_id, dat, sgs_src);
      if xyz.is_some() {
        // let yy = format!("{} -> {}/{} -> {}", self.source_group, sg_id, xyz.unwrap(), self.destination_group);
        let yy = format!("{}/{}, {} -> {}:{}", sg_id, xyz.unwrap(), source.unwrap(), self.destination_ip, self.destination_port);
        st_final.insert(yy);
      }
    }
  }
